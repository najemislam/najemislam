...
export async function GET (req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    let query = supabaseAdmin
      ...
      .select(`
        *,
        user:profiles!community_posts_user_id_fkey(id, full_name, username, avatar_url),
...
export async function POST (req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Check if user is a member OR the community creator
    const [{ data: member }, { data: communityData }] = await Promise.all([
      supabaseAdmin
    ...
    const isCreator = communityData?.creator_id === user_id;
